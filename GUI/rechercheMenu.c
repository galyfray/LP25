...
void rechercheMenu(S_menu* listeMenu,int nbrMenu, int nbMenu){
    ...
    fichier = fopen("liste_menus.txt", "r"); //Ouvre notre.txt de stockage de menus
    int i=0;
    ...
    if(fichier != NULL)
    {
    	...
    	{
        	fscanf(fichier,"/----\n%s\n%s\n%s\n%s\n%s\n%s\n----/", listeMenu[i].menu1, listeMenu[i].menu2, listeMenu[i].menu3, listeMenu[i].menu4, listeMenu[i].menu5, listeMenu[i].menu6); //Stocke les menus dans un tableau de structure correspondant à ceux-ci
